...
doc_types = ['html', 'pdfraw', 'pmc']
scrape_doc_types = ['html', 'pdfraw']
...
def update(pmids=[], ndelay=None, doc_types=doc_types, do_parse=True, overwrite=True):
    ...
    for artidx in range(narts):
        ...
        # Get HTML and PDF
        artdump(artobj.pmid, br, doc_types=doc_types, overwrite=overwrite)

        # Extract PDF text
        if 'pdfraw' in doc_types:
...
def artdump(art, br, doc_types=doc_types, overwrite=False):
    """
    Download documents for an article.
    """
    ...
    for doc_type in doc_types:
        out_files[doc_type] = file_path(art.pmid, doc_type, file_dirs)
    ...
    for doc_type in scrape_doc_types:
        setattr(art, '%smeth' % (doc_type), None)
    art.pdfocr = None
    art.pdfdecrypt = None
    art.pdfdmethod = None
    art.htmlval = None
    ...
    try:
        ...
        art.puburl = puburl
        for doc_type in out_files:
            if out_files[doc_type]:
                setattr(art, '%sfile' % (doc_type), short_name(out_files[doc_type]))
        for doc_type in scrape_doc_types:
            if doc_type in scrape_methods:
                setattr(art, '%smeth' % (doc_type), scrape_methods[doc_type].__name__)
